...
class CSIImagenetfinetune2(Dataset):
    """
    put mini-imagenet files as :
    root :
        |- images/*.jpg includes all imgeas
    ...
    def __init__(self, root, mode, batchsz, n_way, k_shot, k_query, resize, csi_test, csi,location):
        """
...
#             print(self.startidx)
    ...
    def create_batch(self, batchsz):
        ...
        for b in range(batchsz):  # for each batch
            ...
            csi_id = b
            with open(self.csi_test, 'r') as f:
            ...
            # with open('/home/student2/jgao/CSI/MetaLoc/CSIimage0609/csi_test1.csv', 'r') as f:
            #     reader = list(csv.reader(f))  
            #     index = -1
            #     for row in reader:
            #         index = index + 1
            #         if index == csi_id:
            #             #print(row[0])
            #             csi_test1 = cv2.imread(row[0]) 
            # with open('/home/student2/jgao/CSI/MetaLoc/CSIimage0609/csi_test2.csv', 'r') as f:
            #     reader = list(csv.reader(f))  
            #     index = -1
            #     for row in reader:
            #         index = index + 1
            #         if index == csi_id:
            #             #print(row[0])
            #             csi_test2 = cv2.imread(row[0])                      
            # csi_test0 = csi_test0.astype(np.float32)
            # csi_test1 = csi_test1.astype(np.float32)
            # csi_test2 = csi_test2.astype(np.float32)
            # csi_test = (csi_test0+csi_test1+csi_test2)/3
            #csi_test = csi_test.astype(np.uint8)
            sim_list=[]
            for row in range(90):
                with open(self.csi, 'r') as f:
                    reader = list(csv.reader(f))  
                    csi = cv2.imread(reader[row][0])
                    csi = csi.astype(np.float32)
                    #n = self.classify_hist_with_split(csi0, csi_test)
                    #sim_list.append(n) 
                # with open('/home/student2/jgao/CSI/MetaLoc/CSIimage0609/csi1.csv', 'r') as f1:
                #     reader1 = list(csv.reader(f1))
                #     csi1 = cv2.imread(reader1[row][0])
                #     csi1 = csi1.astype(np.float32)
                # with open('/home/student2/jgao/CSI/MetaLoc/CSIimage0609/csi2.csv', 'r') as f2:
                #     reader2 = list(csv.reader(f2))             
                #     csi2 = cv2.imread(reader2[row][0])
                #     csi2 = csi2.astype(np.float32)
                #csi = (csi0+csi1+csi2)/3
                #csi = csi.astype(np.uint8)
